...
import * as snippets from 'app/main/tables/datatables/datatables.snippetcode';
 ...
 */
  ...
  constructor(private router: Router, private runsheetService: RunsheetService, private modalService: NgbModal,
     private _datatablesService: TypographyService,private servicePersonnel : PersonnelService, 
     ...
     ngOnInit() {
      this._datatablesService.onDatatablessChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(async response => {
      this.rows = response;
      console.log(this.rows);
      this.tempData = this.rows;
      this.kitchenSinkRows = this.rows;
      this.exportCSVData = this.rows;
      await this.servicePersonnel.getLivreurList().toPromise().then(
