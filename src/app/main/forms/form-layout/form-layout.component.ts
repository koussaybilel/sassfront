...
/**
          ...
          for (let i=0 ; i< this.rows.length; i++)
            {
              ...
              this.customChkboxOnSelect(this.rows[i], i);
            }
            this.listReferenceForPdf = this.rows.map(item => item.reference);
        }
    else if ( this.test % 2 == 0 )
        {
          for (let i=0 ; i< this.rows.length; i++)
            {
              var element = <HTMLInputElement> document.getElementById(`rowChkbxRef${i}`);
   ...
   * @param { selected }
   */
   customChkboxOnSelect(row, rowIndex) 
   {
    let exists : boolean ;
    let i : number ;
    i = 0;  
    exists = false ; 
    for (i=0; i<this.listrowIndex.length ;i++)
        {
          if (rowIndex == i)
            {
              exists = true; 
    ...
    if (exists == false)
        {
            this.listrowIndex.push(rowIndex);
            this.chkBoxSelected.push(row);
        }
    else if (exists == true)
        {
            this.listrowIndex = this.listrowIndex.filter(item => item != rowIndex);
            this.chkBoxSelected = this.chkBoxSelected.filter(item => item != row);
        }
    
        this.listReferenceForPdf = this.chkBoxSelected.map(item => item.reference);
  }
    ...
    { console.log(this.listReferenceForPdf)
      this.colisService.getBordereauPdf(this.listReferenceForPdf).subscribe(x => {
        ...
        setTimeout(function()  { 
          window.URL.revokeObjectURL(data) ; 
          link.remove 
        }, 100);});for (let i=0 ; i< this.rows.length; i++)
        {
          var element = <HTMLInputElement> document.getElementById(`rowChkbxRef${i}`);
          element.checked = false  ;
          this.chkBoxSelected.pop();//
  ...
  excelRead(e){
    ...
    { 
      ...
      for (let record of this.colisRecords)
        {
          await this.serviceFournisseur.getFournisseurById(this.fournisseurID).toPromise().then(
            (response : Fournisseur)=> {fournisseur= response;},
            (error : HttpErrorResponse) => { alert ( error.message)});
          
          record.fournisseur= fournisseur; 
          await this.colisService.addColis(record).toPromise().then(
            (response : Colis )=> {this._toastrService.success('Vous avez ajouté le colis '+ 
            response.reference +' avec succès ! ','Ajout avec succès !',
            { toastClass: 'toast ngx-toastr', closeButton: true,});
